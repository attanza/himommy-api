import { CreateRoleDto, UpdateRoleDto } from '@/modules/role/role.dto';
...
afterAll(async done => {
  await Role.deleteOne({ name: updateData.name });
  ...
  it('can delete', async () => {
    return request(baseUrl)
      ...
      .expect(async ({ body }) => {
        expect(body.meta).toBeDefined();
        expect(body.meta.status).toEqual(200);
        expect(body.meta.message).toEqual('Role deleted');
      });
  });
});
