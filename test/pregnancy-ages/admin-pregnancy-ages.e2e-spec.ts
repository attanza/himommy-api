...
} from '@/modules/pregnancy-ages/pregnancy-ages.dto';
...
const title = 'Admin Pregnancy-ages';
const baseUrl = 'http://localhost:2500/admin';
const url = '/pregnancy-ages';
const createData: CreatePregnancyAgesDto = {
  week: 37,
...
const updateData: Partial<UpdatePregnancyAgesDto> = {
  isPublish: true,
};
...
afterAll(async done => {
  await PregnancyAges.deleteOne({ name: updateData.title });
  ...
  it('can create', () => {
    return request(baseUrl)
      ...
      .expect(async ({ body }) => {
        ...
        const created = await PregnancyAges.findOne({
  ...
  it('can delete', async () => {
    return request(baseUrl)
      ...
      .expect(async ({ body }) => {
        expect(body.meta).toBeDefined();
        expect(body.meta.status).toEqual(200);
        expect(body.meta.message).toEqual('PregnancyAges deleted');
      });
  });
});
