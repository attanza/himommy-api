...
const title = 'Mobile Pregnancy-ages';
const baseUrl = 'http://localhost:2500/mobile';
const url = '/pregnancy-ages';
let token: string;
let unauthorizedToken: string;
let PregnancyAges: mongoose.Model<mongoose.Document, {}>;
...
describe(`${title} Detail`, () => {
  it('cannot get detail if not authenticated', async () => {
    foundData = await PregnancyAges.findOne();
    return request(baseUrl)
